...
@session_start();
//Database connection
$db = new PDO('mysql:host=localhost;dbname=bitcoin;charset=utf8', 'root', '');
...
function create_user($email, $password, $first_name, $last_name, $username)
{
    ...
    try {
        $stmt = $db->prepare("INSERT INTO auth_user (id,email,password,first_name,last_name,username,date_joined) VALUES(?,?,?,?,?,?,?)");
        $stmt->execute(array($id, $email, $password, $first_name, $last_name, $username,$date_joined));
        $counter = $stmt->rowCount();
